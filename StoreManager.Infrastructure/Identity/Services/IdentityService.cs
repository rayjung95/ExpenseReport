...
using StoreManager.Application.DTOs.Settings;
using StoreManager.Application.Enums;
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _mailService.SendAsync(new MailRequest() { From = "vivien30@ethereal.email", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
